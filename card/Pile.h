...
template <class T> class Pile : public T, public IPile {
 public:
   ...
   virtual void resize(unsigned int pos, PileStyle s) { Check(0); IPile::resize(pos, s); }
...
template <class T> class InfoPile: public Pile<T> {
 public:
   InfoPile(IPile::PileStyle style=IPile::NORMAL, IPile::ShowOpt show=IPile::DONT_CHANGE)
      : Pile<T>(style, show) { }
   virtual ~InfoPile() { }
