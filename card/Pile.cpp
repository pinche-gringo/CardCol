...
IPile::iterator IPile::getFittingCard(const Widget& card, const_iterator start, CMPFUNC2 cmp) const {
   ...
   if (start == end()) {
      ...
      TRACE("IPile::getFittingCard (const Widget*, const_iterator, CMPFUNC2) - Found " << **start);
   }
#endif
...
void IPile::getCards(unsigned int posDest, IPile& src, unsigned int start, int end) {
   TRACE3("IPile::getCards(unsigned int, IPile&, unsigned int, int) - moving from pos " << start << " to " << end);
   Check3(src.size());
