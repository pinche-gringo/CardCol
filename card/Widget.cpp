...
/// Destructor
//-----------------------------------------------------------------------------
Widget::~Widget() { TRACE9("Widget::~Widget() - " << *this); }
...
void Widget::initGestures() {
    ...
    leftClick->signal_released().connect(sigc::mem_fun(*this, &Widget::on_left_released));
    add_controller(leftClick);

    rightClick = Gtk::GestureClick::create();
    rightClick->set_button(GDK_BUTTON_SECONDARY);
    rightClick->signal_released().connect(sigc::mem_fun(*this, &Widget::on_right_released));
...
//-----------------------------------------------------------------------------
void Widget::on_clicked() { TRACE9("Widget::on_clicked() - " << *this); }
...
//-----------------------------------------------------------------------------
Widget* Widget::getEmpty() { return new Widget; }
