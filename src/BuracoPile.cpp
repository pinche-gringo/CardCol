...
//----------------------------------------------------------------------------
bool BuracoPile::getPosition4Card(const Card::Widget& card, unsigned int& pos, unsigned int& move) const {
    TRACE6("BuracoPile::getPosition4Card(const Card::Widget&, 2x unsigned int&) - " << card);
    ...
    if (Buraco::isJoker(card) && ((status.posFirst > 6) || (status.posJoker > 6))) {
        pos =
            ((status.type == NUMBER) ? 1
                                     : ((((status.posFirst < 7)) && (operator[](status.posFirst)->number() == Card::Widget::ACE))
                                            ? (status.posLast + 1)
                                            : 0));
        return true;
    }
    ...
    if (operator[](status.posFirst)->number() == card.number()) {
    ...
    else if ((operator[](status.posFirst)->colour() == card.colour()) && (status.type != NUMBER)) {
        ...
        if ((status.posJoker < 7) &&
            (Buraco::cardDistance(card, *operator[](status.posFirst)) == static_cast<int>(status.posJoker))) {
            if ((status.posJoker && (status.posJoker < status.posLast)) || (card.number() == Card::Widget::ACE))
                move = ((((status.posJoker > status.posFirst) ? operator[](status.posFirst)->number() : card.number()) ==
                         Card::Widget::ACE)
                            ? status.posLast
                            : 0);
            pos = status.posJoker;
        ...
        if ((status.posFirst == status.posLast) || (operator[](status.posFirst)->number() != Card::Widget::ACE)) {
            unsigned int diff(Buraco::cardDistance(*operator[](status.posFirst),
                                                   card, operator[](status.posFirst)->number() < Card::Widget::FIVE));
            TRACE9("BuracoPile::getPosition4Card(const Card::Widget&, 2x unsigned int&) - Diff " << diff << "; max: " << maxDiff);
            Check3(diff);
            ...
            if (diff && (diff <= maxDiff)) {
                ...
                if ((status.posJoker < 7)) {
                    if ((diff == 2) ? ((status.posJoker > status.posFirst) &&
                                       ((operator[](status.posFirst)->number() != Card::Widget::ACE) || (size() < 3)))
                                    : ((card.number() == Card::Widget::ACE)
                                           ? (!status.posJoker && (pos == 1))
                                           : ((card.number() == Card::Widget::KING) && (!pos && status.posJoker)))) {
                        move = ((((status.posJoker > status.posFirst) ? operator[](status.posFirst)->number() : card.number()) ==
                                 Card::Widget::ACE)
                                    ? status.posLast
                                    : 0);
...
void BuracoPile::analysePile() {
    ...
    if (size()) {
        for (const_iterator i(begin()); i != end(); ++i)
            if (Buraco::isJoker(**i)) {
                ...
                ((status.posFirst > 6) ? status.posFirst : status.posLast) = i - begin();
    }
    if (status.posLast > 6)
        status.posLast = status.posFirst;

    status.type = ((status.posFirst != status.posLast)
                       ? ((operator[](status.posFirst)->number() == operator[](status.posLast)->number()) ? NUMBER : COLOUR)
                       : UNDEFINED);
    ...
    if (status.posFirst > 6) {
        ...
        status.points = (((status.type == NUMBER) && (operator[](status.posFirst)->number() == Card::Widget::ACE)) ? 300 : 200);
        if (status.posJoker > 6)
            status.points += 200;
    }

    TRACE8("BuracoPile::analysePile() - " << status.posFirst << '/' << status.posLast << '/' << status.posJoker);
    TRACE9("BuracoPile::analysePile() - Points: " << status.points << ((status.type == NUMBER) ? " (NUMBER)" : " (COLOUR)"));
}
