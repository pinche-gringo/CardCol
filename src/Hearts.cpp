...
bool Hearts::moveSelectedCardToPlayed (unsigned int player, unsigned int card) {
   ...
   if (gameStatus () == PLAYING) {
	 ...
	 if (played.size ()) {
	    ...
	    if (!cardsPlayed) {
	       if ((actCard.colour () != Card::Widget::CLUBS)
		   || (actCard.number () != Card::Widget::TWO))
	 ...
	 if (!cardsPlayed) {
	    if ((actCard.colour () == Card::Widget::SPADES)
		&& (actCard.number () == Card::Widget::QUEEN))
...
void Hearts::getPositionOfColours (Card::IPile& pile, int result[4]) {
   ...
   TRACE9 ("Hearts::getPositionOfColours (Card::IPile&, unsigned int) - Pos. of "
...
unsigned int Hearts::findLowerCard (const Card::IPile& pile, const int aPositions[4]) const {
   ...
   if ((colour == Card::Widget::SPADES)
       && (played[posWinner]->number () > Card::Widget::QUEEN))
   ...
   if ((played.size () == (NUM_PLAYERS - 1))
      ...
      return ((colour != Card::Widget::SPADES)
              || (pile[aPositions[Card::Widget::SPADES]]->number ()
                  != Card::Widget::QUEEN)
              || numberOfCards (aPositions, Card::Widget::SPADES) == 1)
      ...
      TRACE9 ("Hearts::findLowerCard (const Card::IPile&, unsigned int[4]) - Try to"
      ...
      do {
         ...
         if (pile[--card]->number () < highest) {
            ...
            if ((colour == Card::Widget::SPADES)
                && (highest == Card::Widget::ACE)
                && card
                && (pile[card - 1]->number () == Card::Widget::QUEEN)
                && (pile[card - 1]->colour () == Card::Widget::SPADES))
               --card;

            TRACE5 ("Hearts::findLowerCard (const Card::IPile&, unsigned int[4]) - "
      ...
      if ((colour == Card::Widget::SPADES)
          && (card < aPositions[Card::Widget::SPADES])
...
unsigned int Hearts::findWorstCard (const Card::IPile& pile, const int aPositions[4]) const {
   ...
   if (cardsPlayed) {
      TRACE5 ("Hearts::findWorstCard (const Card::IPile&, const int[4]) - Searching"
              " for SQ");
      if (aPositions[2] > 0)
      ...
      TRACE5 ("Hearts::findWorstCard (const Card::IPile&, unsigned int[4]) - "
   ...
   TRACE5 ("Hearts::findWorstCard (const Card::IPile&, unsigned int[4]) - Searching "
           "for high cards");
   int pos (0);
   for (int card (Card::Widget::ACE); card >= Card::Widget::TWO;
      ...
      while ((pos = pile.find (Card::Widget::NUMBERS (card), pos))
             != -1) {
         Card::Widget::COLOURS colour (pile[pos]->colour ());
         TRACE2 ("Hearts::findWorstCard (const Card::IPile&, unsigned int[4]) - "
