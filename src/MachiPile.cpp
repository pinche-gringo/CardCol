...
unsigned int MachiPile::getPosition4Card(const Card::Widget& card) const {
    ...
    if (type != COLOUR) {
        if ((operator[](0)->number() == card.number()) && (find(card.id()) == -1))
            return size();
        else if (type == NUMBER)
            return -1U;
    }
    ...
    if (operator[](0)->colour() == card.colour()) {
        ...
        if (cardDistance(card, *cmp, ((cmp->number() == Card::Widget::ACE) && (size() > 1) ? ONE : BOTH)) == -1)
            return 0;

        cmp = operator[](size() - 1);
        if (cardDistance(card, *cmp, ((cmp->number() == Card::Widget::ACE) && (size() > 1) ? ACE : BOTH)) == 1)
            return size();
    }
...
//----------------------------------------------------------------------------
int MachiPile::cardDistance(const Card::Widget& a, const Card::Widget& b, ACEFLAG aceIsOne) {
    TRACE9("MachiPile::cardDistance(2x const Card::Widget&, ACEFLAG) - " << a << "<->" << b);

    if (a.colour() != b.colour())
        return (a.number() == b.number()) ? 0 : 99;

    if (aceIsOne != ACE) { // Special handling of the ace like 1
        TRACE9("MachiPile::cardDistance(2x const Card::Widget&, ACEFLAG) - Checking for Ace");
        if (a.number() == Card::Widget::ACE) {
            if ((aceIsOne == ONE) || ((b.number() < Card::Widget::FOUR) && (b.number() != Card::Widget::ACE)))
                return -static_cast<int>(b.number()) - 1;
        }
        else if (b.number() == Card::Widget::ACE)
            if ((aceIsOne == ONE) || ((a.number() < Card::Widget::FOUR) && (a.number() != Card::Widget::ACE)))
                return static_cast<int>(a.number()) + 1;
    }

    TRACE9("MachiPile::cardDistance(2x const Card::Widget&, ACEFLAG) - Distance: " << a.number() - b.number());
...
void MachiPile::analysePile() {
    if (size() == 2)
        type = ((operator[](0)->number() == operator[](1)->number()) ? NUMBER : COLOUR);
    else if (size() < 2)
        type = UNDEFINED;
}
...
void MachiPile::checkIntegrity() const {
    ...
    for (const_iterator i(begin()); (i + 1) != end(); ++i)
        if ((type == COLOUR)
                ? (((*i)->colour() != (*(i + 1))->colour()) || (cardDistance(**(i + 1), **i, (i == begin()) ? ONE : ACE) != 1))
                : ((*i)->number() != (*(i + 1))->number())) {
            Glib::ustring error(_("Card %1 does not fit!"));
            error.replace(error.find("%1"), 2, YGP::ANumeric::toString((i + 1) - begin()));
...
///     found!
//----------------------------------------------------------------------------
bool MachiPile::hasMatching3rd(std::vector<Card::Widget*>& pair, MachiPile::const_iterator& match, unsigned int& nr) const {
    Check1(pair.size() == 2);
    ...
    int diffTable(cardDistance(*pair[0], *card,
                               (diff < 0)                                                              ? ONE
                               : ((pair[0]->number() == Card::Widget::ACE) && (pair[1]->number() < 5)) ? ACE
                                                                                                       : BOTH));
    TRACE1("MachiPile::hasMatching3rd(ICardPile&) - Differences: " << diff << '/' << diffTable);
    ...
    case 0: // Equal numbers
        if (diffTable) {
            ...
            if (getType() == NUMBER) {
                match = begin();
                while (match != end()) {
                    if ((pair[0]->id() != (*match)->id()) && (pair[1]->id() != (*match)->id()))
                        break;
                    ++match;
                }
            }
    ...
    case 1:
        TRACE9("MachiPile::hasMatching3rd(ICardPile&) - Checking " << *pair[0] << '-' << *pair[1] << '-' << *card);
        if ((pair[0]->colour() == card->colour()) && (getType() == COLOUR)) {
    ...
    TRACE5("MachiPile::hasMatching3rd(ICardPile&) - Match " << ((match != end()) ? 'Y' : 'N') << "; " << nr);
    return match != end();
}
