...
//-----------------------------------------------------------------------------
Buraco::Buraco(Gtk::Box& parent, Gtk::Statusbar& statusbar, Card::Set& cardset,
...
unsigned int Buraco::cardFitsOnPlayedPile(unsigned int player, unsigned int iCard) {
   ...
   for (std::vector<BuracoPile*>::iterator p(tablePiles[player & 1].begin()); p != tablePiles[player & 1].end(); ++p) {
      TRACE5("Buraco::cardFitsOnPlayedPile(unsigned int, unsigned int) - Checking pile " <<(int)(p - tablePiles[player & 1].begin()));
      Check3(*p);
   ...
   if (pile.getPosition4Card(card, pos, move)) {
      ...
      if ((pile.size() > 1) || isJoker(card))
         ...
         do {
            ...
            TRACE8("Buraco::cardFitsOnPile(unsigned int, const Card::Widget&) const -  Dist: " << dist << "<->" << cardDistance(**pCard, card));
